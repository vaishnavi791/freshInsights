...
def dashboard_data():
    ...
    # ✅ FIX: Convert NaN to None for JSON serialization
    ...
    if len(df_iot) > 0:
        # Temperature & Humidity charts (only add if data exists)
        ...
        # (Temperature distribution, humidity distribution, etc.)
    ...
    # 2. Temperature Distribution
        ...
        color_map = {'Unripe': '#90EE90', 'Ripe': '#FFD700', 'Overripe': '#FF6347'}
        ...
        charts['qualityStatus'] = {
            'optimal': 20 <= avg_temp <= 25 and 50 <= avg_hum <= 70,
            'acceptable': 15 <= avg_temp <= 30 and 40 <= avg_hum <= 80,
        }
    ...
    # ✅ Clean raw data for JSON
    raw_data_cleaned = df.to_dict('records')
    for record in raw_data_cleaned:
